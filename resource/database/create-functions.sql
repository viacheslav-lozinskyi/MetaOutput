...
BEGIN
    IF (NOT ISNULL(__netId)) THEN
        ...
        IF (NOT EXISTS(SELECT _id FROM net_sessions WHERE netId = __netId LIMIT 1)) THEN
            ...
            IF (NOT ISNULL(__ref)) THEN
                SET @__context1 = LOWER(__ref);
                SELECT campaignId FROM net_campaigns WHERE @__context1 LIKE pattern LIMIT 1 INTO @__context;
                #SET __campaignContent = REPLACE(REPLACE(REPLACE(__ref, "https://", ""), "http://", ""), "/", "");
                #SET __campaignTerm = DATE_FORMAT(CURRENT_TIMESTAMP, "%Y-%m-%d");
            END IF;
            
            INSERT INTO net_sessions (netId, country, city, organization, browser, os, resolution, language, ref, campaignId, campaignName, campaignSource, campaignMedium, campaignTerm, campaignContent)
            VALUE (__netId, __country, __city, __organization, __browser, __os, __resolution, __language, __ref, @__context, LOWER(__campaignName), LOWER(__campaignSource), LOWER(__campaignMedium), LOWER(__campaignTerm), LOWER(__campaignContent));
        ELSE
            SET SQL_SAFE_UPDATES = 0;
...
BEGIN
    IF (NOT ISNULL(__name) AND NOT ISNULL(__source) AND NOT ISNULL(__medium)) THEN
        ...
        IF (ISNULL(__campaignId) OR (NOT ISNULL(__campaignId) AND NOT EXISTS(SELECT _id FROM net_campaigns WHERE (name = __name) AND (source = __source) AND (medium = __medium) AND (pattern = __pattern)))) THEN
